...
<script type="text/javascript">
    ...
    function jqalert(param) {
        var title = param.title,
            ...
            price = param.price,
            status = param.status,
            str = `<a style="text-decoration:none" class="ml-5" href="index.php?module=distribution&action=view&id=${id}" title="查看">
        ...
        $("#yes_btn").click( function () {
            if(status != 1){
                 window.location.href="index.php?module=distribution&action=set";
            }
...
<span class="c-gray en">&gt;</span>  
 <a href="index.php?module=distribution&action=Indexreturn&status=4">退货列表</a>
...
<div class="pd-20">
    ...
    <div class="status qh {if $status==3}isclick{/if}" ><a href="index.php?module=distribution&action=Indexorder&status=3">订单列表</a></div>
    <div class="status qh {if $status==4}isclick{/if}" style="border-radius: 2px 0px 0px 2px !important;"><a href="index.php?module=distribution&action=Indexreturn&status=4">退货列表</a></div>
...
<input type="hidden" class="price" value="">
    <div class="text-c"> 
        <form name="form1" action="index.php" method="get">
            ...
            <input type="hidden" name="action" value="Indexreturn" />
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            {foreach from=$list item=item name=f1}
                <tr class="text-c">   
                    ...
                    <td width="250px"> 
                        <a style="text-decoration:none;" class="ml-5" href="index.php?module=distribution&action=view&id={$item->id}" title="查看">
                        	<div style="align-items: center;font-size: 12px;display: flex;">
                            	<div style="margin: 0 auto;display: flex;align-items: center;">
                                <img src="images/icon1/ck.png"/>&nbsp;查看
                                </div>
                        ...
                        {if $item->r_type == 0}
                            {if $item->re_type == 1}
                                {if $status1 == 1}  <!-- 有地址 -->
                                    <a style="text-decoration:none" class="ml-5" href="javascript:;" title="审核通过" onclick="is_ok(this,{$item->id},1,'确定要通过该用户的申请,并让用户寄回?')">
                                        <div style="align-items: center;font-size: 12px;display: flex;">
                                            <div style="margin: 0 auto;display: flex;align-items: center;">
                                            <img src="images/icon1/qy.png"/>&nbsp;通过5
                                            </div>
                                        </div>
                                    </a>
                                {else}
                            ...
                            {elseif $item->re_type == 2}
                                <a style="text-decoration:none" class="ml-5" href="javascript:;" title="审核通过" onclick="is_ok(this,{$item->id},9,'确定要通过该用户的申请,并将钱款原路返还?')">
                                    <div style="align-items: center;font-size: 12px;display: flex;">
	                                    <div style="margin: 0 auto;display: flex;align-items: center;">
	                                    <img src="images/icon1/qy.png"/>&nbsp;通过4
	                                    </div>
                                    </div>
                                </a>
                            ...
                            {else}
                                ...
                                {if $status1 == 1}  <!-- 有地址 -->
                                    <a style="text-decoration:none" class="ml-5" href="javascript:;" title="审核通过" onclick="is_ok(this,{$item->id},6,'确定要通过该用户的申请,并让用户寄回?')"><div style="align-items: center;font-size: 12px;display: flex;">
                                        <div style="margin: 0 auto;display: flex;align-items: center;">
                                        <img src="images/icon1/qy.png"/>&nbsp;通过3
                                        </div>
                                        </div>
                                    </a>
                                {else}
                                    ...
                                    <a style="text-decoration:none" class="ml-5" href="javascript:;" title="审核通过" onclick="is_add(this,{$item->id},6,',请添加售后地址')"><div style="align-items: center;font-size: 12px;display: flex;">
                                        <div style="margin: 0 auto;display: flex;align-items: center;">
                                        <img src="images/icon1/qy.png"/>&nbsp;通过2
                                        </div>
                                        </div>
                                    </a>
                                {/if}
                        ...
                        {elseif $item->r_type == 3}
                            ...
                            {if $item->re_type < 3}
                             <a style="text-decoration:none" class="ml-5" href="javascript:;" title="同意并退款" onclick="is_ok(this,{$item->id},4,'确定已到货并退款到用户?')">
                             	<div style="align-items: center;font-size: 12px;display: flex;">
                                    <div style="margin: 0 auto;display: flex;align-items: center;">
                                    <img src="images/icon1/qy.png"/>&nbsp;通过1
                                    </div>
                                    </div>
                                </a>
                            {else}
...
function refuse(obj,id,type) {
    jqalert({
        ...
        notext:'取消',
        id:id,
        url:"index.php?module=distribution&action=examine",
        obj:obj,
        type:type,
        status:1
    })
...
function zxfahuo(sNo,id) {
     layer_show('添加快递信息','index.php?module=distribution&action=addsignreturn&id='+id+'&sNo='+sNo,600,400);
}
...
function is_add(obj,id,type,content) {  
		...
		function closeMask(id){
			 $.get("index.php?module=distribution&action=del1",{'id':id},function(res){
                    if(res.status=="1"){
                       appendMask("删除成功","cg");
                    }else{
                        appendMask("删除失败","ts");
