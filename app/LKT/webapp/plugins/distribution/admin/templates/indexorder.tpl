...
<nav class="breadcrumb" style="font-size: 16px;"><span class="c-gray en"></span><i class="Hui-iconfont">&#xe67f;</i> 分销管理
  ...
  <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a>
	...
	<div class="pd-20">
	<div style="display: flex;flex-direction: row;font-size: 16px;margin-bottom: 10px;" class="page_bgcolor">
    <div class="status qh {if $status==2}isclick{/if}"  ><a href="index.php?module=distribution&action=goods&status=2">分销商品</a></div>
        <div class="status qh {if $status==3}isclick{/if}" style="border-radius: 2px 0px 0px 2px !important;"><a href="index.php?module=distribution&action=Indexorder&status=3">订单列表</a></div>
        <div class="status qh {if $status==4}isclick{/if}"><a href="index.php?module=distribution&action=Indexreturn&status=4">退货列表</a></div>
    <div class="status qh {if $status==0}isclick{/if}"  ><a href="index.php?module=distribution&action=index&status=0">佣金明细</a></div>
    <div class="status qh {if $status==1}isclick{/if}"><a href="index.php?module=distribution&action=relationship&status=1">客户关系</a></div>
  </div>
		<div class="text-c">
			<form name="form1" id="form1" action="index.php" method="get">

				<input type="hidden" name="module" value="distribution" />
				<input type="hidden" name="action" value="Indexorder" />
		...
		<td class="tab_dat">
			...
			<a class="hover_a" onclick="navto('index.php?module=distribution&action=Detail&id={$item->id}')" title="订单详情">
				<img src="images/icon1/ck.png" />&nbsp;订单详情
			</a>
			{if $item->statu < 3} <a class="hover_a"
				onclick="navto('index.php?module=distribution&action=Modifyorder&id={$item->id}&type=updata')" title="编辑订单">
	...
	<script type="text/javascript">
		...
		function colse(sNo) {
			colsEsno = sNo
			deIsOpn = 2
			close1('确认关闭此订单？', { colsEsno, deIsOpn })
		}
		...
		$(".qd").click(function () {
			...
			if (express_name.length < 1) {
				...
				$.ajax({
					url: "index.php?module=distribution&action=addsign",
					type: "post",
		...
		function send_btn(obj, otype, id, status, sNo) {
			...
			if (stu == 1) {
				// $(".dc").show();
				location.href = 'index.php?module=distribution&action=addsign&sNo=' + dingdan
			}
		...
		function send_btn1(obj, id, courier_num, is = false) {
			...
			$.ajax({
				url: 'index.php?module=distribution&action=kuaidishow&r_sNo=' + r_sNo + '&courier_num=' + courier_num,
				type: "post",
		...
		$('#getAllSelectedId').click(function () {
			...
			if (ids.length === 0) {
				...
				layer.confirm('确定要删除吗？', function (index) {
					...
					var delIds = ids.join(",");
					$.ajax({
						url: "index.php?module=distribution&action=delorder",
						type: "post",
						data: {
							ids: delIds
						},
		...
		function deleteS() {
			$.ajax({
				url: 'index.php?module=distribution&action=delorder&ids=' + conStr,
				type: "post",
				data: {},
				success: function (res) {
					// 刷新
		...
		// 关闭订单
		function openS(colsEsno) {
			$.ajax({
				url: 'index.php?module=distribution&action=addsign&m=close&sNo=' + colsEsno,
				type: "post",
				data: {},
				success: function (res) {
					location.replace(location.href);
			...
			$("body").append(`
				<div class="maskNew">
					<div class="maskNewContent">
						...
						<div style="text-align:center;margin-top:30px">
							<button class="closeMask" style="margin-right:20px" onclick=openS('${id.colsEsno}') >确认</button>
							<button class="closeMask" onclick=closeMask1() >取消</button>
						</div>
					</div>
				</div>
