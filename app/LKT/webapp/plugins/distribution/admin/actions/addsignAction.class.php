...
class addsignAction extends Action
{
	...
	public function execute()
	{
		...
		if (!empty($express_id)) {
			...
			$db->rollback();
			echo json_encode(array('code' => 400, 'message' => '请选择快递公司！'));
			exit();
		}
		if (!empty($courier_num)) {
			...
			if ($rr) {
				$db->rollback();
				echo json_encode(array('code' => 400, 'message' => '快递单号已被使用，请重新添加！'));
				// echo 0;
				exit();
			} else {
			...
			$db->rollback();
			echo json_encode(array('code' => 400, 'message' => '请添加快递单号！'));
			exit();
		}
		...
		if ($otype == 'yb') {
			...
			if ($id) {
				...
				if ($rr == $r011) { //全部子订单发货完全改变lkt_order表
	...
	public function close($sNo)
	{
		...
		// print_r("select z_price,sNo,status,coupon_id,consumer_money,user_id from lkt_order  where sNo='" . $sNo . "'");die;
		if ($r) {
			$db->begin();
			...
			// print_r("select sid,num,p_id from lkt_order_details where r_sNo = $sNo");
			// print_r($ss);
			if ($ss) {
			...
			if ($rl > 0 && $rd > 0) {
				...
				$db->rollback();
				echo json_encode(array('status' => 0, 'err' => '操作失败2!'));
				exit();
			}
		} else {
			echo json_encode(array('status' => 0, 'err' => '操作失败1!'));
			exit();
		}
	}
}
