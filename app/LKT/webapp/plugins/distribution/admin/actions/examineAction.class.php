        ...
        if ($m == 1 || $m == 4 || $m == 9) {
            ...
            if ($m == 9 || $m == 4) {
                ...
                if ($order_res) {
                    ...
                    $sNo = $order_res[0]->sNo;
                    //返佣金
                    $re01 = $db->select("select * from lkt_detailed_commission where sNo = '$sNo' and status = 1 and recycle = 0 and pid =$p_id ");
                    ...
                    if ($re01) {
                        foreach ($re01 as $key => $value) {
                            ...
                            $rrr = $db->insert("insert into lkt_detailed_commission(userid,sNo,money,s_money,status,type,Referee,addtime,pid) values('$userid','$sNo','$money','$s_money','3','$type','$Referee',CURRENT_TIMESTAMP),$p_id");
                        }
                    }
            ...
            if ($m == 8) { //拒绝申请
                ...
                // 根据订单号,修改订单详情状态
                $sql_d = "update lkt_order_details set r_status = '1',r_content = '$text' where id = '$id' ";
                // print_r($sql_d);die;
                $res2 = $db->update($sql_d);
                $sql_u = "update lkt_order set status = '1' where sNo = '$sNo' ";
                $res1 = $db->update($sql_u);
                // $rr = $db -> selectrow("select id from lkt_order_details where r_sNo='" . $sNo ."'");
                //  $r011 = $db -> selectrow("select id from lkt_order_details where r_sNo='" . $sNo ."' and r_status >=2");
                //  if($rr == 1){//全部子订单发货完全改变lkt_order表
                //      $sqll = 'update lkt_order set status=1 where sNo="' . $sNo . '"';
                //          $rl = $db -> update($sqll);
                //  }
