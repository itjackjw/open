...
class IndexreturnAction extends Action
{
    ...
    public function getDefaultView()
    {
        ...
        $status = $request->getParameter('status'); // 升/降

        $r_type = trim($request->getParameter('r_type'));


        $condition = ' r_status = 4 and a.r_sNo=b.sNo"';
        if ($p_name != '') {
        ...
        // 查询插件表
        $sql1 = "select a.* from lkt_order_details as a ,lkt_detailed_commission as b where  $condition";
        ...
        $url = 'index.php?module=distribution&action=indexreturn' . $con;
        $pages_show = $pager->multipage($url, $total, $page, $pagesize, $start, $para = '');


        if ($pageto == 'all') { // 导出全部
            $sql = "select a.* from lkt_order_details as a ,lkt_detailed_commission as b where $condition order by $sort_name $sort ";
            $r = $db->select($sql);
        } else if ($pageto == 'ne') { // 导出本页
            $sql = "select a.* from lkt_order_details as a ,lkt_detailed_commission as b where $condition order by $sort_name $sort limit $start,$pagesize ";
            $r = $db->select($sql);
        } else {
            $sql = "select a.* from lkt_order_details as a ,lkt_detailed_commission as b where $condition limit $start,$pagesize ";
