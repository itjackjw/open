...
class ModifyAction extends Action {
  ...
  public function getDefaultView() {
    ...
    // $data.freight+$data.z_price-$reduce_price-$coupon_price
    $data['z_price'] =$o_price-$reduce_price-$coupon_price;
   if($data['otype'] == 'pt'){
...
//佣金信息
  ...
  public function execute(){
     ...
     $y_price = addslashes(trim($request->getParameter('sz_price')));//原价
     $price =  $z_price-$y_price;
     ...
     if($price>=0){//增加价格
        $sql = 'update lkt_order set z_price=z_price+"'.$price.'",name="'.$name.'",mobile="'.$mobile.'",address="'.$address.'" where sNo="'.$sNo.'"';  
